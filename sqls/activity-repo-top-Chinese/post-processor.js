module.exports = async function(data,config) {
  ...
  config.orgs.forEach(item =>{
    org_id_company_map.set(item.id,item.company);
  });
  config.repos.forEach(item =>{
    repo_id_company_map.set(item.id,item.company);
  });
  ...
  //sort by activity
  let company_info_list=new Array();//{["company",repo_activity,issue_comment...]}
  company_map.forEach((info,company)=>{
    company_info_list.push(company);
    info.forEach(item=>{
      company_info_list.push(item);
    });
  });
