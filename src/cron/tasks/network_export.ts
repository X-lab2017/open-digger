...
`;
    ...
    const activityProp = (type: string) => months.map(m => `COALESCE(${type}.activity_${m}, 0.0)`).join('+');

    const processData = async (rows: { n1: string; n2: string; rel: number }[], name: string, fileName: string) => {
      const set = new Set<string>();
      rows.forEach(r => set.add(r.n1));
      rows.forEach(r => set.add(r.n2));
      const data = {
        nodes: Array.from(set).map<any>(name => [name, openrankMap.get(name) ?? 1]).sort((a, b) => b[1] - a[1]),
        ...
        edges: rows.map<any>(r => [r.n1, r.n2, parseFloat(r.rel.toFixed(2))]).filter(n => n[0] != n[1]).sort((a, b) => b[2] - a[2]),
      };
      (async () => {
...
GROUP BY actor_id`);
      ...
      for (const [id, login] of users) {
        ...
        await processData(await query(repoNetworkSql, { id: parseInt(id), limit }), login, repoNetworkFile);
...
ORDER BY act DESC
LIMIT toInteger($limit)
WITH collect(u.id) AS uid, collect(u.name) AS uname
CALL {
  WITH uid
  MATCH (u1:User)-[a1:ACTION]->(r:Repo)<-[a2:ACTION]-(u2:User)
  WHERE u1.id IN uid AND u2.id IN uid AND u1.id > u2.id AND ${activityProp('a1')} > 0 AND ${activityProp('a2')} > 0
...
GROUP BY repo_id`);
      ...
      for (const [id, name] of repos) {
        await processData(await query(userNetworkSql, { id: parseInt(id), limit }), name, developerNetworkFile);
    ...
    await generateUserNetwork();
    await generateRepoNetwork();
  }
};
