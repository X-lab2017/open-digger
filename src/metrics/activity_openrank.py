...
def get_RepoActivityOrOpenrank(config, type='activity'):
    """_summary_
    ...
    if not config.groupBy:
        query = 'MATCH (r:Repo) WHERE {} {} RETURN r.name AS repo_name, r.org_login AS org, [{}] AS {} ORDER BY reverse({}) {} {};'.format(repoWhereClause+' AND ' if repoWhereClause else '', timeWhereClause, ','.join(timeActivityOrOpenrankClause), type, type, config.order, 'LIMIT {}'.format(config.limit) if config.limit > 0 else '')
        return neo4j_driver.query(query)
    elif config.groupBy == 'org':
        query = 'MATCH (r:Repo) WHERE {} {} RETURN r.org_login AS org_login, count(r.id) AS repo_count, [{}] AS {} ORDER BY reverse({}) {} {};'.format(repoWhereClause+' AND ' if repoWhereClause else '', timeWhereClause, list(map(lambda i:'round(SUM({}), {})'.format(i, config.percision)), timeActivityOrOpenrankClause), type, type, config.order, 'LIMIT {}'.format(config.limit) if config.limit > 0 else '')
        return neo4j_driver.query(query)
    else:
        ...
        labelData = filter(lambda l: l.type == config.groupBy, getLabelData()) if getLabelData() != None else None 
        result = {}
        if labelData == None: return None
        for row in queryResult:
            label = [l for l in labelData if l.githubRepos.includes(row.repo_id) or l.githubOrgs.includes(row.org_id)][0]
            if label == None: return None
            if not label.name in result: values = row[type]
            else:
            ...
            result[label.name] = {
                'label': label.name,
                'repo_count': (result.get(label.name)['repo_count'] if label.name in result else 0) + 1,
                [type]: values,
            }
        resultArr = [result.values()]
        if config.order == 'ASC': resultArr.sort(key = cmp_to_key(lambda a, b: a[type][len(a[type]) - 1] - b[type][len(b[type]) - 1]))
        if config.order == 'DESC': resultArr.sort(key = cmp_to_key(lambda a, b: b[type][len(b[type]) - 1] - a[type][len(a[type]) - 1]))
        for i in resultArr:
            i[type] = list(map(lambda v: np.around(np.array(v,dtype=i[type]), config.percision)))
        return resultArr[0:config.limit]

def get_UserActivityOrOpenrank(config, type='activity'):
    """_summary_
    ...
    query = 'MATCH (u:User) WHERE {} {} RETURN u.login AS user_login, [{}] AS {} ORDER BY {} {} {};'.format(userWhereClause +' AND ' if userWhereClause else '', timeWhereClause, ','.join(timeActivityClause), type, type, config.order, 'LIMIT {}'.format(config.limit) if config.limit > 0 else '')
    return neo4j_driver.query(query)
...
def get_RepoActivityWithDetail(config):
    config = getMergedConfig(config)
  ...
  FORMAT JSONCompact".format(ISSUE_COMMENT_WEIGHT, OPEN_ISSUE_WEIGHT, OPEN_PULL_WEIGHT, REVIEW_COMMENT_WEIGHT,
                             'ORDER BY activity DESC LIMIT {} BY time'.format(config.limit) if config.limit > 0 else '',
                             config.order
...
def get_UserActivityWithDetail(config = QueryConfig, withBot = True):
    config = getMergedConfig(config)
    whereClauses = ["type IN ('IssuesEvent', 'IssueCommentEvent', 'PullRequestEvent', 'PullRequestReviewCommentEvent')"] # specify types to reduce memory usage and calculation
    userWhereClause = getUserWhereClauseForClickhouse(config)
    if userWhereClause: whereClauses.append(userWhereClause)
    whereClauses.append(getTimeRangeWhereClauseForClickhouse(config))
    sql = "SELECT id, argMax(name, time) AS name,\
   {},\
   {},\
