...
export const chaossChangeRequestsAcceptedRation = async (config: QueryConfig) => {
  config = getMergedConfig(config);
...
(
  SELECT
    ${getGroupTimeAndIdClauseForClickhouse(config, 'repo')},
    count(case when pull_merged = 1 then 1 end) AS accepted
    count(case when pull_merged = 0 then 1 end) AS declined
  FROM github_log.events
  WHERE ${whereClauses.join(' AND ')}
  GROUP BY id, time
