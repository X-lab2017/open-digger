...
def getRepoOpenrank(config):
    ...
    timeActivityOrOpenrankClause = getTimeRangeSumClauseForNeo4j(
        config, 'r.{}'.format(calType))
    if not config.get('groupBy'):
        query = 'MATCH (r:Repo) WHERE {} {} RETURN r.name AS repo_name, r.org_login AS org, [{}] AS {} ORDER BY reverse({}) {} {};'.format(
            repoWhereClause+' AND ' if repoWhereClause else '', timeWhereClause, ','.join(timeActivityOrOpenrankClause), calType, calType, config.get('order'), 'LIMIT {}'.format(config.get('limit')) if config.get('limit') > 0 else '')
        return neo4j_driver.query(query)
    elif config.get('groupBy') == 'org':
        query = 'MATCH (r:Repo) WHERE {} {} RETURN r.org_login AS org_login, count(r.id) AS repo_count, [{}] AS {} ORDER BY reverse({}) {} {};'.format(repoWhereClause+' AND ' if repoWhereClause else '', timeWhereClause, list(
            map(lambda i: 'round(SUM({}), {})'.format(i, config.get('percision')), timeActivityOrOpenrankClause)), calType, calType, config.get('order'), 'LIMIT {}'.format(config.get('limit')) if config.get('limit') > 0 else '')
        return neo4j_driver.query(query)
    else:
        query = 'MATCH (r:Repo) WHERE {} {} RETURN r.id AS repo_id, r.org_id AS org_id, [{}] AS {};'.format(
            repoWhereClause + ' AND ' if repoWhereClause else '', timeWhereClause, ','.join(timeActivityOrOpenrankClause), calType)
        queryResult = neo4j_driver.query(query)
        labelData = list(filter(lambda l: l.get('type') == config.get(
            'groupBy'), getLabelData())) if getLabelData() != None else None
        result = {}
        if labelData == None:
            return None
        for row in queryResult:
            labels = list(filter(lambda l: int(row.get('repo_id')) in l.get(
                'githubRepos') or int(row.get('org_id')) in l.get('githubOrgs'), labelData))
            for label in labels:
                if not label.get('name') in result.keys():
                    values = row[calType]
                else:
        ...
        if config.get('order') == 'ASC':
            resultArr.sort(key=cmp_to_key(lambda a, b: a[calType][len(
                a[calType]) - 1] - b[calType][len(b[calType]) - 1]))
        if config.get('order') == 'DESC':
            resultArr.sort(key=cmp_to_key(lambda a, b: b[calType][len(
                b[calType]) - 1] - a[calType][len(a[calType]) - 1]))
        for i in resultArr:
...
def getUserOpenrank(config):
    ...
    timeActivityClause = getTimeRangeSumClauseForNeo4j(
        config, 'u.{}'.format(calType))
    query = 'MATCH (u:User) WHERE {} {} RETURN u.login AS user_login, [{}] AS {} ORDER BY {} {} {};'.format(userWhereClause + ' AND ' if userWhereClause else '', timeWhereClause, ','.join(
        timeActivityClause), calType, calType, config.get('order'), 'LIMIT {}'.format(config.get('limit')) if config.get('limit') > 0 else '')
    return neo4j_driver.query(query)
...
basicActivitySqlComponent = f''' 
        ...
        countIf(type=\'PullRequestEvent\' AND ((action=\'closed\' AND pull_merged=1) OR (action=\'merged\'))) AS merged_pull, 
...
def getRepoActivity(config):
    config = getMergedConfig(config)
    # specify types to reduce memory usage and calculation
    whereClauses = [
  ...
  (".format(getGroupArrayInsertAtClauseForClickhouse(config, {'key': 'activity', 'defaultValue': '0'}),
            getGroupArrayInsertAtClauseForClickhouse(
                config, {'key': 'issue_comment', 'defaultValue': '0'}),
            getGroupArrayInsertAtClauseForClickhouse(
                config, {'key': 'open_issue', 'defaultValue': '0'}),
            getGroupArrayInsertAtClauseForClickhouse(
                config, {'key': 'open_pull', 'defaultValue': '0'}),
            getGroupArrayInsertAtClauseForClickhouse(
                config, {'key': 'review_comment', 'defaultValue': '0'}),
            getGroupArrayInsertAtClauseForClickhouse(
                config, {'key': 'merged_pull', 'defaultValue': '0'}),
            ) + \
        "SELECT \
      {}, \
  ...
  FORMAT JSONCompact".format(ISSUE_COMMENT_WEIGHT, OPEN_ISSUE_WEIGHT,
                             ...
                             'ORDER BY activity DESC LIMIT {} BY time'.format(
                                 config.get('limit')) if config.get('limit') > 0 else '',
                             config.get('order')
...
FORMAT JSONCompact".format(ISSUE_COMMENT_WEIGHT, OPEN_ISSUE_WEIGHT, OPEN_PULL_WEIGHT, REVIEW_COMMENT_WEIGHT, PULL_MERGED_WEIGHT,
                           ' AND '.join(
                               whereClauses), '' if withBot else 'AND actor_login NOT LIKE \'%[bot]\'',
                           'ORDER BY activity DESC LIMIT {} BY time'.format(
                               config.get('limit')) if config.get('limit') > 0 else '',
                           config.get('order'))
