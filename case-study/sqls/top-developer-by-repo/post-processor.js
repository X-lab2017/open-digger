...
| # | login | activity | issue_comment | open_issue | open_pull | pull_review_comment | merge_pull | pull_commits | pull_additions | pull_deletions |\n|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|\n`;
    d.sort((a, b) => {
      return b.develpoer_activity - a.develpoer_activity;
    });
    d.slice(0, config.topN).forEach((item, index) => {
      ret += `| ${index + 1} | ${item.actor_login} | ${item.develpoer_activity} |  ${item.issue_comment} | ${item.open_issue} | ${item.open_pull} | ${item.pull_review_comment} | ${item.merge_pull} | ${item.pull_commits} | ${item.pull_additions} | ${item.pull_deletions} |\n`;
    });
  }
  return ret;
}
