...
def __processAppendRatio(row, cloumns, countIndex=-1, totalCountIndex=-2):
    ...
    processResult['ratio'] = list(map(lambda v: '{}%'.format(
...
def chaossCodeChangeCommits(config, mode='outer') -> (List, str):
    ...
    generated_sql = Chaoss.__bulidOuterCountSql(
        config, inner_sql, 'commits_count')
...
def __chaossCount(config, mode, whereClauses, countColName) -> (List, str):
    ...
    if mode == 'origin':
        ...
        return rst, inner_sql
    generated_sql = Chaoss.__bulidOuterCountSql(
        config, inner_sql, countColName)
    columns = ['id', 'name', 'total_count', 'count', 'ratio']
    rst = Chaoss.__executeOuterSql(
        generated_sql, columns, Chaoss.__processAppendRatio)
    return rst, generated_sql
...
def chaossBusFactor(config, mode='outer') -> (List, str):
    ...
    if mode == 'origin':
        columns = ['time', 'id', 'name',
                   'total_contributions', 'bus_factor', 'detail']
...
def __chaossResolutionDuration(config, type, mode) -> (List, str):
    ...
    unit = filterEnumType(config.get("options", {}).get(
        "unit"), Chaoss.timeDurationConstants["unitArray"], 'day')
    thresholds = config.get("options", {}).get("thresholds", [3, 7, 15])
    ranges = thresholds + [-1]
    sortBy = filterEnumType(config.get("options", {}).get(
        "sortBy"), Chaoss.timeDurationConstants["sortByArray"], 'avg')
    ...
    '''
    if mode == 'origin':
        columns = ['time', 'id', 'name', 'avg', 'quantile_0', 'quantile_1',
                   'quantile_2', 'quantile_3', 'quantile_4', 'resolution_levels']
        rst = Chaoss.__executeInnnerSql(inner_sql, columns)
        return rst, inner_sql
    ...
    '''
    columns = ['id', 'name', 'resolution_duration_avg', 'levels',
               'quantile_0', 'quantile_1', 'quantile_2', 'quantile_3', 'quantile_4']
...
def __chaossResponseTime(config, type, mode) -> (List, str):
    ...
    if type == 'issue':
        whereClauses.append(
            "type IN ('IssueCommentEvent', 'IssuesEvent') AND actor_login NOT LIKE '%[bot]'")
    else:
        whereClauses.append(
            "type IN ('IssueCommentEvent', 'PullRequestEvent', 'PullRequestReviewCommentEvent', 'PullRequestReviewEvent') AND actor_login NOT LIKE '%[bot]'")
    ...
    if mode == 'origin':
        columns = ['time', 'id', 'name', 'avg', 'quantile_0', 'quantile_1',
                   'quantile_2', 'quantile_3', 'quantile_4', 'response_levels']
        rst = Chaoss.__executeInnnerSql(inner_sql, columns)
        return rst, inner_sql
    ...
    {getOutterOrderAndLimit(config, sortBy, 1 if sortBy == 'levels' else None)}
    '''
    columns = ['id', 'name', 'response_time_avg', 'levels', 'quantile_0',
...
def __chaossAge(config, type, mode) -> (List, str):
    ...
    if mode == 'origin':
        columns = ['time', 'id', 'name', 'avg', 'quantile_0', 'quantile_1',
                   'quantile_2', 'quantile_3', 'quantile_4', 'age_levels']
        rst = Chaoss.__executeInnnerSql(inner_sql, columns)
        return rst, inner_sql
...
def chaossChangeRequestsAcceptanceRatio(config, mode='outer') -> (List, str):
    ...
    '''
    if mode == 'origin':
        columns = ['time', 'id', 'name', 'count',
                   'accepted_count', 'declined_count', 'ratio']
        rst = Chaoss.__executeInnnerSql(inner_sql, columns)
        return rst, inner_sql
...
def chaossNewContributors(config, mode='outer') -> (List, str):
    config = getMergedConfig(config)
    by = filterEnumType(config.get('options').get('by') if config.get(
        'options') != None else None, ['commit', 'change request'], 'change request')
    whereClauses = []
    ...
    '''
    columns = ['id', 'name', 'new_contributors',
               'detail', 'total_new_contributors']
    rst = Chaoss.__executeOuterSql(generated_sql, columns, Chaoss.__process)
    return rst, generated_sql
...
def chaossInactiveContributors(config, mode='outer') -> (List, str):
    config = getMergedConfig(config)
    by = filterEnumType(config.get("options", {}).get(
        'by'), ['commit', 'change request'], 'change request')
    timeInterval = config.get("options", {}).get('timeInterval', 6)
    timeIntervalUnit = filterEnumType(config.get("options", {}).get(
        'timeIntervalUnit'), ['month', 'quarter', 'year'], 'month')
